...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 bg-white" id="sidenav-main">
  ...
  <div class="collapse navbar-collapse  w-auto h-auto" id="sidenav-collapse-main">
    ...
    <li class="nav-item mt-2">
      ...
      @endif
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('registered-assets') ? 'active' : '') }}" href="{{ url('registered-assets') }}">
            <div class="{{ (Request::is('registered-assets') ? 'bg-gradient-info' : 'icon') }} icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fas fa-user-check ps-2 pe-2 text-center text-dark {{ (Request::is('registered-assets') ? 'text-white opacity-10' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            @if(Auth::user()->type == 1)
              <span class="nav-link-text ms-1">Registered Assets</span>
            @endif
            @if(Auth::user()->type == 2)
      ...
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('reports') || Request::is('issues') ? 'active' : '') }}" href="{{ url('reports') }}">
            <div class="{{ (Request::is('reports') || Request::is('issues') ? 'bg-gradient-info' : 'icon') }} icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fas fa-flag ps-2 pe-2 text-center text-dark {{ (Request::is('reports') || Request::is('issues') ? 'text-white opacity-10' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            <span class="nav-link-text ms-1">Report Issues</span>
            <span class="ms-4 mt-1 translate-middle badge rounded-pill bg-danger">{{ $count_reports }}</span>
        </a>
      </li>
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('rooms') || Request::is('semester') || Request::is('room-schedule') ? 'active' : '') }}" href="{{ url('rooms') }}">
            <div class="{{ (Request::is('rooms') || Request::is('semester') || Request::is('room-schedule') ? 'bg-gradient-info' : 'icon') }} icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fa-house-user ps-2 pe-2 text-center text-dark {{ (Request::is('rooms') || Request::is('semester') || Request::is('room-schedule') ? 'text-white opacity-10' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            <span class="nav-link-text ms-1">Room Schedule</span>
        </a>
      </li>
      @if(Auth::user()->type == 2)
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('unschedule-class') ? 'active' : '') }}" href="{{ url('unschedule-class') }}">
            <div class="{{ (Request::is('unschedule-class') ? 'bg-gradient-info' : 'icon') }} icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fa-business-time ps-2 pe-2 text-center text-dark {{ (Request::is('unschedule-class') ? 'text-white opacity-10' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            <span class="nav-link-text ms-1">Unschedule Class</span>
        </a>
      </li>
      @endif
      @if(Auth::user()->type == 1)
