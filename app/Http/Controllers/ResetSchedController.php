...
class ResetSchedController extends Controller
{
    ...
    public function schedule() {
        ...
        if(Auth::user()->type == 1) {
            return Schedule::where('date_from', 'like', '%'.date('Y-m-d').'%')->orderBy('created_at', 'DESC')->get();
        }

        if(Auth::user()->type == 2) {
            return Schedule::where('date_from', 'like', '%'.date('Y-m-d').'%')->where(['userid' => Auth::user()->userid])->orderBy('created_at', 'DESC')->get();
        }
